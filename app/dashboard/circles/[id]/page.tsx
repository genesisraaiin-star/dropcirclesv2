...
export default function EditCirclePage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (updateError) {
      console.error('Failed to link file:', updateError);
    } else {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center">
      <p className="text-gray-400 text-sm font-mono tracking-widest uppercase animate-pulse">Unlocking...</p>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#FAFAFA] text-black p-8 md:p-16 font-sans">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-12">
          ...
          <p className="text-gray-500 text-sm">Upload your audio and edit drop details.</p>
        </div>

        {/* Audio Uploader */}
        <div className="bg-white border border-dashed border-gray-200 rounded-xl p-8 mb-8 text-center">
          {circle.assets && circle.assets.length > 0 ? (
            <div className="flex flex-col items-center">
              <div className="w-12 h-12 bg-green-50 text-green-500 rounded-full flex items-center justify-center mb-4 text-lg">✓</div>
              <h3 className="text-sm font-medium mb-1">Track uploaded</h3>
              <p className="text-xs text-gray-500 mb-4">{circle.assets.length} file{circle.assets.length > 1 ? 's' : ''} in vault</p>
              <div className="relative inline-block">
                ...
                />
                <button disabled={uploading} className="text-xs border border-gray-200 text-gray-600 px-4 py-2 rounded hover:border-gray-400 transition-colors disabled:opacity-50">
                  {uploading ? 'Uploading...' : 'Replace / add file'}
                </button>
              </div>
            </div>
          ) : (
        ...
        <form onSubmit={handleSaveDetails} className="space-y-6 mb-12">
          ...
          <div>
            <label className="text-[10px] text-gray-400 font-medium tracking-widest uppercase mb-2 block">Description</label>
            <textarea
              name="description"
              rows={4}
          ...
          <button
            type="submit"
            disabled={saving}
            className="w-full bg-black text-white p-4 rounded-lg text-sm font-medium hover:bg-gray-800 transition-colors disabled:opacity-50"
          >
        ...
        <div className="border-t border-red-100 pt-8 mt-16">
          ...
          <p className="text-gray-500 text-xs mb-4">Permanently deletes this drop and all audio files. Fans immediately lose access.</p>
          <button
            onClick={handleDelete}
            className="text-xs border border-red-200 text-red-600 px-4 py-2 rounded hover:bg-red-50 transition-colors"
          >
            Destroy forever
          </button>
        </div>
