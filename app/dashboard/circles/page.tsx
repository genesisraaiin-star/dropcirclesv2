...
export default function CirclesDashboard() {
  ...
  if (loading) return <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center font-sans"><p className="text-gray-400 text-sm font-medium tracking-widest uppercase animate-pulse">Unlocking vault...</p></div>;
    ...
    <div className="min-h-screen bg-[#FAFAFA] text-black p-8 md:p-16 font-sans">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-4">
          ...
          <Link href="/dashboard/circles/create" className="bg-black text-white px-6 py-3 rounded-lg font-medium hover:bg-gray-800 transition-colors inline-flex items-center gap-2 text-sm">
        ...
        {circles && circles.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {circles.map((circle) => {
              ...
              return (
                <div key={circle.id} className={`bg-white border ${isLocked ? 'border-red-100 opacity-75' : 'border-gray-200'} rounded-xl p-6 hover:shadow-sm transition-all flex flex-col h-full relative overflow-hidden`}>
                  ...
                  <div className="pt-4 border-t border-gray-100 flex flex-col gap-4 mt-auto z-10">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      {/* 🔥 HERE IS THE MISSING EDIT BUTTON 🔥 */}
                      <div className="flex items-center gap-4">
                        <Link href={`/dashboard/circles/${circle.id}`} className="text-[10px] uppercase tracking-widest text-gray-500 hover:text-black transition-colors font-bold underline">
                          EDIT ↗
                        </Link>

                        {!circle.expires_at && circle.is_active && (
                          <button onClick={() => triggerSelfDestruct(circle.id)} className="text-[10px] bg-red-50 text-red-600 px-2 py-1 rounded uppercase tracking-widest hover:bg-red-100 transition-colors">
                            Set 24h Drop
                          </button>
                        )}
                    ...
                    <div className="flex justify-between items-center bg-gray-50 p-2 rounded-lg">
                      ...
                      <button onClick={() => toggleActiveStatus(circle.id, circle.is_active)} className={`w-11 h-6 rounded-full transition-colors relative ${circle.is_active ? 'bg-black' : 'bg-gray-200'}`}>
          ...
          <div className="text-center py-24 border border-dashed border-gray-300 rounded-xl bg-white">
            ...
            <Link href="/dashboard/circles/create" className="text-black underline text-sm hover:text-gray-600 transition-colors">Create your first drop</Link>
          </div>
        )}
